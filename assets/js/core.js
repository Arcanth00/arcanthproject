...
function updateAuthUI(user) {
    ...
    const userProfile = g('user-profile');
    if (!loginBtn || !userProfile) return;

    if (user) {
        loginBtn.classList.add('hidden');
        userProfile.classList.remove('hidden');
        g('header-user-avatar').src = user.photoURL || `https://ui-avatars.com/api/?name=${user.displayName || 'U'}&background=random`;
        g('header-user-name').textContent = user.displayName || 'Kullanıcı';
        g('header-user-email').textContent = user.email;
    } else {
...
function openLoginModal() { g('login-modal').classList.remove('hidden'); }
function closeLoginModal() { g('login-modal').classList.add('hidden'); }
...
function checkMaintenanceStatus() {
    if (window.location.pathname.includes('bakim.html') || window.location.pathname.includes('admin_panel.html')) return;

    db.ref('site_data/settings/maintenance').on('value', snap => {
        const data = snap.val();
        if (data && data.enabled) {
            ...
            auth.onAuthStateChanged(user => {
                if (user && allowedAdmins.includes(user.uid)) {
                    ...
                    // Show a small warning toast for admin
                    const toast = document.createElement('div');
                    toast.className = 'fixed bottom-4 left-4 z-[1000] bg-red-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl animate-bounce';
                    toast.innerHTML = '⚠️ BAKIM MODU AKTİF';
...
function toggleMobileMenu() {
    ...
    if (h) {
        const lines = h.querySelectorAll('.hamburger-line');
        if (lines.length === 3) {
...
function performSearch(e) {
    ...
    r.innerHTML = results.map(i => `
        <div class="search-result-item" onclick="scrollToSection('${i.page}');closeSearch();">
...
function acceptCookies() {
    ...
    if (n) n.classList.add('translate-y-full');
}
