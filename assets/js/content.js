...
function loadCMSContent() {
    ...
    if (d.hero) {
        if (d.hero.badge && g('hero-badge-text')) g('hero-badge-text').textContent = d.hero.badge;
        if (d.hero.title && g('h-title')) {
            const words = d.hero.title.split(' ');
            const lastWord = words.pop();
            g('h-title').innerHTML = `<span class="dark:text-white block">${words.join(' ')}</span><span class="text-gradient block mt-4">${lastWord}.</span>`;
        }
    ...
    if (localStorage.getItem('cookies-accepted') === 'true') {
        ...
        setTimeout(() => {
            ...
            if (n) {
                ...
                if (config.title && g('cookie-title')) g('cookie-title').textContent = config.title;
                if (config.description && g('cookie-description')) g('cookie-description').textContent = config.description;
                if (config.acceptText && g('cookie-accept-btn')) g('cookie-accept-btn').textContent = config.acceptText.toUpperCase();
                if (config.rejectText && g('cookie-reject-btn')) g('cookie-reject-btn').textContent = config.rejectText.toUpperCase();
